              ...
              <div class="card mb-4">
                ...
                <div class="btn-group mb-1 col-md-1">
                  <button type="button" class="btn btn-info dropdown-toggle" data-toggle="dropdown"
                    style="color: #ffffff;" aria-haspopup="true" aria-expanded="false">
                    {{ records }}
                  </button>
                  ...
                  <table class="table align-items-center table-flush table-hover" id="dataTableHover">
                    ...
                    <tbody>
                      <tr v-for="product in subProducts" :key="product.id">
                        ...
                        <td><span class="recOverflow" style="width: 100px;">{{ product.buying_price }} $</span></td>
                        <td> <span class="recOverflow">{{ product.selling_price }} $</span></td>
                        <td> {{ product.root }}</td>
                        <td>
    ...
    recordsLength() {
      this.filteredProducts = this.products.filter(product => {
        if (product.name.toLowerCase().match(this.searchTerm.toLowerCase())
          ...
          || product.root.toLowerCase().match(this.searchTerm.toLowerCase()))
          return product;
      });
